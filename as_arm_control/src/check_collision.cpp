...
bool CheckCollisionNode::CheckCollision(vector<double>& values) {
    ...
    collision_detection::AllowedCollisionMatrix acm = this->m_planScene->getAllowedCollisionMatrixNonConst();
    robot_state::RobotState copied_state = this->m_planScene->getCurrentState();
    collision_detection::CollisionResult::ContactMap::const_iterator it2;
    int k=0;
    for(it2=this->m_collResult.contacts.begin();
        it2!=this->m_collResult.contacts.end();
        ++it2,k++){
    ...
    m_planScene->checkCollision(this->m_collRequest, this->m_collResult, copied_state, acm);
